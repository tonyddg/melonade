...
$
  n_i (x)=cases(product_(k=0)^(i)(x-x_k)\,&i>0,1\,&i=0),space N(x)=sum_(i=0)^(#hl(2)[$n-1$]) c_i n_i (x) 
$
...
#problem_box(
  title: [Newton 插值例题],
  problem: [
    已知 $f(x)=5x^2(3x-2)(2x+1)$ 设互异节点 $x_i,i=0,1,2,3,4$ 求 (1) $sum^4_(i=0)l_i (x)$, (2) $f[x_0,x_1,x_2,x_3,x_4]$
  ]
)[
  注意题目没有给出节点值，且 $f(x)$ 为 4 次多项式与节点个数相同

  (1) 根据 $l_i (x_j)=cases(1\,i=j,0\,i eq.not j)$ 可得对于 $q(x)=sum^4_(i=0)l_i (x)$ 有 $q(x_i)=1$，对五个节点使用牛顿插值可以得到 $q[x_0]=1,q[x_0,x_1]=0,dots,q[x_0,dots,x_4]=0$, 因此 $q(x)=1$

  (2) 由于节点个数与多项式相同，有插值结果 $N(x)=f(x)$，注意到 $N(x)=sum_(i=0)^(n-1) c_i n_i (x)$ 中，$n_3 (x)$ 是其中唯一四次多项式且四次项系数为 $1$，而 $c_i=f[x_0,dots,x_4]$，因此 $f[x_0,dots,x_4]$ 即 $f(x)$ 的四次项系数 $30$
]
...
具体使用时
- 先将给定函数 $f(x)$ 中的变量换元为 $x=m(t)$ 得到 $h(t)=f[m(t)]$ 的最佳平方逼近 $p(t)$
...
#problem_box(
  ...
  一次多项式中有 Legendre 多项式 $P_0 (x)=1,P_1 (x)=x$，求基函数自身内积和与原函数间内积有（根号中的多项式可通过换元发转为单个变量）
...
#problem_box(
  ...
  以上解法适用于计算机求解的情况，实际利用#link(<sec:least_square>)[正规方程]求最小二乘解即可
  ...
  有期望等式两边最小二乘误差最小的正规方程（建议逐个计算上三角部分的元素）

  $
    mat(1,-2,4;1,-1,1;1,0,0;1,1,1;1,2,4)arrow(a)=mat(0;1;2;1;0) arrow mat(1,1,1,1,1;-2,-1,0,1,2;4,1,0,1,4) mat(1,-2,4;1,-1,1;1,0,0;1,1,1;1,2,4)arrow(a)=mat(1,1,1,1,1;-2,-1,0,1,2;4,1,0,1,4)mat(0;1;2;1;0) arrow arrow(a)=mat(58/7;0;-3/7)
  $
...
对于 Lagrange 插值有 $c_i=f(x_i)$，此时#hl(2)[求积公式中的系数 $a_i$ 满足
...
对于线性方程 $bm(A)arrow(x)=arrow(b)$ 可以改写为等价形式 $arrow(x)=bm(B)arrow(x)+arrow(f)$，即可得到线性方程一般迭代公式。对于 $m times m$ 的方阵，相比计算复杂度为 $m^3$ 的矩阵求逆，以下迭代公式的计算复杂度为 $k m^2$。

$
  arrow(x)^((k+1))=bm(B)arrow(x)^((k))+arrow(f)
$
...
#problem_box(
  ...
  $
    ...
    bm(B)_G=-(bm(D)+bm(L))^(-1)bm(U)=mat(0,-2,2;0,2,-3;0,0,2), arrow(f)_G=(bm(D)+bm(L))^(-1)arrow(b)=mat(1;0;-1)\ 
  $
...
不动点迭代法收敛时将满足如下极限，因此一般情况下不动点迭代法收敛速度为 $p=1$ 的线性收敛，当 $phi(x^*)=0$ 时有 $p=2$ 的平方收敛（其中 $x^*$ 为方程的解）
...
因此 Newton 迭代法在收敛时一般具有 $p=2$ 的平方收敛速度。但 Newton 迭代法不一定收敛，依然需要依据 $abs(phi(x))lt.eq 1$ 确定收敛性。
